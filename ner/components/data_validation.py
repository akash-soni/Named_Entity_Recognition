...
class DataValidation:
    ...
    def type_check(self) -> bool:
        try:
            ...
            for split_name in self.data_validation_config.data_split:
                count = 0
                for column in self.data_validation_config.columns_check:
                    if self.data[split_name].features[column].feature.dtype in self.data_validation_config.type_check:
                        count += 1
                type_check_result.append(count)
            logger.info(f" Type Check Results {type_check_result}")

            if sum(type_check_result) == len(self.data_validation_config.columns_check) * 3:
                return True
            else:
                return False
        ...
        try:
            ...
            for split_name in self.data_validation_config.data_split:
                null_check_result.append(
                    pd.DataFrame(self.data[split_name]).isnull().values.any()
                )
            logger.info(f" Type Check Results {null_check_result}")
            print(null_check_result)

            if sum(null_check_result) == 0:
                return True
            else:
                return False
